  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 font-sans pb-8 px-4">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100">Order Details: {order.poNumber || 'Unnamed PO'}</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-neutral-200 dark:border-slate-700 p-6 space-y-4">
          <h2 className="text-lg font-semibold flex items-center gap-2 border-b border-neutral-200 dark:border-slate-700 pb-3 text-neutral-800 dark:text-neutral-200"><FileText className="h-5 w-5 text-blue-600" /> Purchase Order Information</h2>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-neutral-500 dark:text-neutral-400">PO Number</p>
              <p className="font-medium text-neutral-900 dark:text-neutral-100">{order.poNumber || '-'}</p>
            </div>
            <div>
              <p className="text-neutral-500 dark:text-neutral-400">Customer Name</p>
              <p className="font-medium text-neutral-900 dark:text-neutral-100">{order.customerName || '-'}</p>
            </div>
            <div>
        ...
        {/* Addresses */}
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-neutral-200 dark:border-slate-700 p-6 space-y-4 md:col-span-2">
          <h2 className="text-lg font-semibold flex items-center gap-2 border-b border-neutral-200 dark:border-slate-700 pb-3 text-neutral-800 dark:text-neutral-200"><MapPin className="h-5 w-5 text-indigo-600" /> Addresses</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div className="space-y-2">
              <h3 className="font-medium text-neutral-900 dark:text-neutral-100">Billing Address</h3>
              <p className="text-neutral-700 dark:text-neutral-300 whitespace-pre-line">{order.billingAddress || '-'}</p>
              <div className="grid grid-cols-2 gap-2 mt-2 text-neutral-800 dark:text-neutral-200">
                <div><span className="text-neutral-500 dark:text-neutral-400">PIN:</span> {order.billingPinCode || '-'}</div>
                <div><span className="text-neutral-500 dark:text-neutral-400">Contact:</span> {order.billingContact || '-'}</div>
                <div className="col-span-2"><span className="text-neutral-500 dark:text-neutral-400">Email:</span> {order.billingEmail || '-'}</div>
              </div>
            </div>
            <div className="space-y-2">
              <h3 className="font-medium text-neutral-900 dark:text-neutral-100">Delivery Address</h3>
              <p className="text-neutral-700 dark:text-neutral-300 whitespace-pre-line">{order.deliveryAddress || '-'}</p>
              <div className="grid grid-cols-2 gap-2 mt-2 text-neutral-800 dark:text-neutral-200">
                <div><span className="text-neutral-500 dark:text-neutral-400">PIN:</span> {order.deliveryPinCode || '-'}</div>
                <div><span className="text-neutral-500 dark:text-neutral-400">Contact:</span> {order.deliveryContact || '-'}</div>
                <div className="col-span-2"><span className="text-neutral-500 dark:text-neutral-400">Email:</span> {order.deliveryEmail || '-'}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-slate-800">
              {order.specs && order.specs.filter((s:any) => s.sku || s.size || s.design || s.quantity > 0).map((spec: any, i: number) => (
                <tr key={i} className="hover:bg-neutral-50/50 dark:hover:bg-slate-800 transition-colors text-neutral-900 dark:text-neutral-100">
              ...
              {(!order.specs || order.specs.filter((s:any) => s.sku || s.size || s.design || s.quantity > 0).length === 0) && (
                <tr>
                  <td colSpan={4} className="px-4 py-3 text-sm text-center text-neutral-500 dark:text-neutral-400">No specifications found</td>
                </tr>
              )}
            </tbody>
          </table>
