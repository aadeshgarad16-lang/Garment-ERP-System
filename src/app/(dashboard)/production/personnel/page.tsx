    ...
    <div className="max-w-7xl mx-auto space-y-6 font-sans pb-8 px-4 sm:px-6 lg:px-8 pt-6">
      ...
      {personnelList.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {personnelList.map((person, i) => (
            <div key={i} className="bg-white dark:bg-slate-900 border border-neutral-100 dark:border-slate-800 rounded-2xl shadow-sm p-5 flex flex-col gap-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-neutral-800 dark:text-neutral-200">{person.name}</h3>
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 font-mono mt-0.5">
                    {person.contact}
                  </p>
                </div>
              </div>
              <div className="bg-neutral-50 dark:bg-slate-800/50 rounded-xl p-4 space-y-3 border border-neutral-100 dark:border-slate-700 flex-1">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-neutral-500 dark:text-neutral-400">Assigned Stage</span>
                  <span className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-[10px] font-bold rounded uppercase tracking-wider">{person.activeStage}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-neutral-500 dark:text-neutral-400">Active PO Batch</span>
                  <span className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">{person.poBatch}</span>
                </div>
                <div className="flex justify-between items-center border-t border-neutral-200 dark:border-slate-700 pt-3 mt-3">
        ...
        <div className="w-full border-2 border-dashed border-neutral-300 dark:border-slate-700 rounded-xl p-10 flex flex-col items-center justify-center text-center bg-neutral-50/50 dark:bg-slate-800/20">
          <div className="p-4 bg-white dark:bg-slate-900 rounded-full shadow-sm mb-4 border border-neutral-100 dark:border-slate-800">
            <User className="h-8 w-8 text-neutral-400" />
          </div>
          <p className="text-neutral-800 dark:text-neutral-200 font-bold text-lg mb-1">No personnel deployed on the floor</p>
          <p className="text-sm text-neutral-500 dark:text-neutral-400 max-w-sm">Click '+ Add Person' to assign workers.</p>
        </div>
      )}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-neutral-200 dark:border-slate-800">
              <h3 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">Add New Personnel</h3>
              <button 
                onClick={() => setIsOpen(false)}
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 border border-neutral-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-shadow bg-transparent"
                />
              </div>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full px-3 py-2 border border-neutral-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-shadow bg-transparent text-neutral-700 dark:text-neutral-200"
                >
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-neutral-200 dark:border-slate-800 bg-neutral-50 dark:bg-slate-800/50">
              <button 
                onClick={() => setIsOpen(false)}
