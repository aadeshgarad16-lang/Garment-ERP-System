...
export default function MaterialReleasePage() {
  ...
  const handleReleaseToProduction = () => {
    ...
    setGlobalMessage({ text: 'Materials released successfully and sent to production.', type: 'success' });
  ...
  let releaseStatusText = 'Awaiting Release';
  if (allReleased) releaseStatusText = 'Fully Released';
  else if (releasedCount > 0) releaseStatusText = 'Partially Released';
    ...
    <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6 font-sans pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Issue frozen warehouse inventory to the production floor</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-5 md:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div>
          <span className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><FileText className="h-3.5 w-3.5" /> Release ID</span>
          <p className="text-sm font-semibold text-indigo-900 py-1.5">REL-2026-042</p>
        </div>
        <div>
          <span className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><FileText className="h-3.5 w-3.5" /> Linked PO</span>
          <p className="text-sm font-semibold text-neutral-900 py-1.5">PO-2026-004</p>
        </div>
        <div>
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><Clock className="h-3.5 w-3.5" /> Release Date</label>
        ...
        <div>
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><Clock className="h-3.5 w-3.5" /> Release Time</label>
        ...
        <div>
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><User className="h-3.5 w-3.5" /> Store Manager</label>
          <input type="text" placeholder="Manager Name" className="w-full px-3 py-1.5 text-sm text-neutral-800 placeholder:text-neutral-400 border rounded-lg border-neutral-300 focus:ring-indigo-500 focus:border-indigo-500" value={releaseInfo.storeManager} onChange={e => setReleaseInfo({...releaseInfo, storeManager: e.target.value})} disabled={releaseCompleted} />
        </div>
        <div>
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><User className="h-3.5 w-3.5" /> Floor Supervisor</label>
          <input type="text" placeholder="Supervisor Name" className="w-full px-3 py-1.5 text-sm text-neutral-800 placeholder:text-neutral-400 border rounded-lg border-neutral-300 focus:ring-indigo-500 focus:border-indigo-500" value={releaseInfo.floorSupervisor} onChange={e => setReleaseInfo({...releaseInfo, floorSupervisor: e.target.value})} disabled={releaseCompleted} />
        </div>
        <div className="md:col-span-2">
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><Building2 className="h-3.5 w-3.5" /> Production Floor</label>
          <select className="w-full px-3 py-1.5 text-sm text-neutral-800 placeholder:text-neutral-400 border rounded-lg border-neutral-300 focus:ring-indigo-500 focus:border-indigo-500 bg-white" value={releaseInfo.productionFloor} onChange={e => setReleaseInfo({...releaseInfo, productionFloor: e.target.value})} disabled={releaseCompleted}>
            ...
            <option value="Floor 1 - Cutting">Floor 1 - Cutting</option>
            <option value="Floor 2 - Stitching">Floor 2 - Stitching</option>
            <option value="Floor 3 - Finishing">Floor 3 - Finishing</option>
            <option value="Floor 4 - Quality Control">Floor 4 - Quality Control</option>
          </select>
        </div>
        <div className="md:col-span-2 lg:col-span-4">
          <label className="text-xs font-medium text-neutral-500 flex items-center gap-1 mb-1"><FileText className="h-3.5 w-3.5" /> Notes (Optional)</label>
          <input type="text" placeholder="Any special instructions or comments for the production floor" className="w-full px-3 py-1.5 text-sm text-neutral-800 placeholder:text-neutral-400 border rounded-lg border-neutral-300 focus:ring-indigo-500 focus:border-indigo-500" value={releaseInfo.notes} onChange={e => setReleaseInfo({...releaseInfo, notes: e.target.value})} disabled={releaseCompleted} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4 sm:p-5 lg:p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-500">Total Frozen Materials</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4 sm:p-5 lg:p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-500">Released Materials</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4 sm:p-5 lg:p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-500">Pending Release</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-500">Release Status</p>
      ...
      {releaseCompleted ? (
        <div className="bg-white rounded-xl shadow-sm border border-emerald-200 overflow-hidden mt-6">
          <div className="border-b border-emerald-100 px-6 py-5 bg-emerald-50/50 flex flex-col items-center text-center">
            ...
            <h2 className="text-xl font-bold text-emerald-800">Materials Released Successfully</h2>
            <p className="text-emerald-600 text-sm mt-1">Inventory has been issued to the production floor.</p>
          </div>
          
          <div className="p-6">
            <h3 className="text-md font-bold text-neutral-800 mb-4">Release Summary</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="p-4 bg-neutral-50 rounded-lg border border-neutral-100">
                <p className="text-xs text-neutral-500 mb-1">Release ID</p>
              ...
              <div className="p-4 bg-neutral-50 rounded-lg border border-neutral-100">
                <p className="text-xs text-neutral-500 mb-1">Production Floor</p>
                <p className="font-semibold text-neutral-900">{releaseInfo.productionFloor || 'Floor 1 - Cutting'}</p>
              </div>
              <div className="p-4 bg-neutral-50 rounded-lg border border-neutral-100">
            ...
            <h3 className="text-md font-bold text-neutral-800 mb-4">Released Materials</h3>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-neutral-50 border-b border-neutral-100 text-xs uppercase tracking-wider text-neutral-500 font-medium">
                    ...
                    <th className="px-6 py-3 text-right">Released Qty</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100">
                  {mockFrozenMaterials.filter(item => releaseStates[item.id].releaseQty > 0).map(item => (
                    <tr key={item.id}>
                      <td className="px-6 py-3 text-sm text-neutral-900">{item.name}</td>
                      <td className="px-6 py-3 text-sm font-medium text-neutral-900 text-right">{releaseStates[item.id].releaseQty} {item.unit}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
            <div className="border-b border-neutral-200 px-6 py-5 bg-neutral-50/50">
              <h2 className="text-lg font-semibold text-neutral-800">Issue Inventory to Production</h2>
            </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap min-w-[900px]">
            <thead>
              <tr className="bg-white border-b border-neutral-100 text-xs uppercase tracking-wider text-neutral-500 font-medium">
                ...
                <th className="px-6 py-4 text-right">Frozen Qty</th>
                <th className="px-6 py-4">Destination</th>
                <th className="px-6 py-4 text-center">Release Qty</th>
                ...
                return (
                  <tr key={item.id} className={`transition-colors ${st.status === 'Released to Production' ? 'bg-emerald-50/30' : 'hover:bg-neutral-50/80'}`}>
                    ...
                    <td className="px-6 py-4">
                      ...
                      {isError && <p className="text-[10px] text-red-600 mt-1 text-center font-medium">Exceeds frozen</p>}
                    </td>
                    <td className="px-6 py-4">
