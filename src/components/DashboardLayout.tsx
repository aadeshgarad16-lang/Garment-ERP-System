...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // ONE-TIME GLOBAL PURGE: Ensure legacy mock data is fully wiped on root mount
    const purgeKey = 'mock_purge_completed_v1';
    if (!localStorage.getItem(purgeKey)) {
      console.log('Performing one-time global purge of legacy mock cache keys...');
      localStorage.clear();
      localStorage.setItem(purgeKey, 'true');
      
      // Preserve auth token if they had one so they aren't forced to re-login immediately, 
      // but otherwise, wipe the rest. 
      // Note: `localStorage.clear()` removes auth as well. To strictly remove just dummy data:
      const dummyKeys = ['procurement_orders', 'store_procurement_orders', 'procurement_history', 'savedOrders', 'bomCalculationDraft', 'autoGeneratedProcurementRequests', 'preStitchedInventory', 'systemLogs', 'archivedProcurementRequests'];
