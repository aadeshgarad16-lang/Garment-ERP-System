        ...
        <div className="space-y-6">
          ...
          >
            ...
            {podImage ? (
              ...
              <>
                <div className="h-14 w-14 bg-card rounded-full flex items-center justify-center shadow-sm border border-border mb-4 group-hover:scale-110 transition-transform">
                  <UploadCloud className="h-7 w-7 text-blue-500" />
                </div>
                <h3 className="text-sm font-semibold text-card-foreground mb-1">{t('logistics.uploadPod') || 'Upload POD Document'}</h3>
                <p className="text-xs text-muted-foreground">{t('orderInitiation.orderForm.maxSize') || 'Supports JPG, PNG, PDF (Max 5MB)'}</p>
              </>
            )}
