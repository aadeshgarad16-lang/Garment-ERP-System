  ...
  return (
    <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="border-b border-border px-6 py-5 bg-neutral-50/50 dark:bg-slate-800/60 dark:border-slate-800 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-card-foreground dark:text-white">{t('orderInitiation.tracker.verification') || 'Packing & Verification'}</h2>
          <p className="text-xs text-muted-foreground dark:text-slate-300 mt-0.5">{t('quality.stages.verification.desc') || 'Verify packed quantities against shipping manifest before final approval.'}</p>
        </div>
      </div>
